...
//       existing.role = role;
//       existing.district = role === "student" ? normalizeDistrict(district) : "";
...
//   return res.status(501).json({ message: "Optional / keep your existing" });
// };
...
const sendOtpWhatsAppOnly = async ({ phone, otp }) => {
  const msg = `Your Lesi Iskole verification code is: ${otp} (valid for ${OTP_TTL_MINUTES} minutes)`;
...
export const signUp = async (req, res) => {
  try {
    ...
    if (existing && !existing.isVerified) {
      ...
      await sendOtpWhatsAppOnly({
        phone: normalizedPhone,
        otp,
      });

      return res.status(200).json({
        message: "User already exists but not verified. OTP re-sent by WhatsApp.",
        user: safeUser(existing),
      });
    }
    ...
    return res.status(201).json({
      message: "User created. OTP sent by WhatsApp.",
      user: safeUser(user),
    });
  } catch (err) {
    console.error("signUp error:", err);
    ...
    if (err?.message === "WhatsApp OTP send failed") {
      return res.status(500).json({
        message: "User created, but WhatsApp OTP sending failed. Please try resend code.",
      });
    }
...
export const sendVerificationCode = async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: "OTP sent by WhatsApp" });
  } catch (err) {
    console.error("sendVerificationCode error:", err);

    if (err?.message === "WhatsApp OTP send failed") {
      return res.status(500).json({
        message: "WhatsApp OTP sending failed. Please try again.",
      });
    }
