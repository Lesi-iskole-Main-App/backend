...
const sendOtpSMSOnly = async ({ phone, otp }) => {
  ...
  try {
    ...
    throw new Error("SMS OTP send failed");
  }
};
...
export const signUp = async (req, res) => {
  try {
    ...
    if (existing && !existing.isVerified) {
      ...
      await sendOtpSMSOnly({
        phone: normalizedPhone,
        otp,
      });

      return res.status(200).json({
        message: "User already exists but not verified. OTP re-sent by SMS.",
    ...
    return res.status(201).json({
      message: "User created. OTP sent by SMS.",
      user: safeUser(user),
    });
  } catch (err) {
    ...
    if (err?.message === "SMS OTP send failed") {
      return res.status(500).json({
        message: "User created, but SMS OTP sending failed. Please try resend code.",
      });
    }
...
export const sendVerificationCode = async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: "OTP sent by SMS" });
  } catch (err) {
    console.error("sendVerificationCode error:", err);

    if (err?.message === "SMS OTP send failed") {
      return res.status(500).json({
        message: "SMS OTP sending failed. Please try again.",
      });
    }
